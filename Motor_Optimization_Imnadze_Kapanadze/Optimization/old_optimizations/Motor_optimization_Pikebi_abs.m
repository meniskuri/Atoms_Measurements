...
f                  = data_meas(:, 1);
%% lokaluri maximi da minimumi wertilebis povna gazomili monacemebis abs_istvis
w     = [data_meas(:,1) abs(data_meas(:,2) + 1i*data_meas(:,3))];
x_lok = w(2,2);
y_lok = 0;
f_lok = 0;
n     = 0;
...
for n=3:1:length(w)
    ...
    if n==3 && (x_lok<w(1,2)||x_lok>w(1,2))
        ...
        count_index(count) = 1;
        count = count + 1;
    elseif n==length(w) && (x_lok<w(length(w),2)||x_lok>w(length(w),2))
        y_lok(count,1) = w(length(w),2);
        f_lok(count,1) = w(length(w),1);
        x_lok = w(n,2);
        count_index(count) = length(w);
        count = count + 1;
    else
        if (x_lok>w(n-2,2)&&x_lok>w(n,2)||(x_lok<w(n-2,2)&&x_lok<w(n,2)))
            y_lok(count,1) = w(n-1,2);
            f_lok(count,1) = w(n-1,1);
            x_lok = w(n,2);
            count_index(count) = n-1; % <== count index !!!!
            count = count + 1;
        else
            x_lok = w(n,2);
        end
...
xlabel('Frequency')
ylabel('/Omega')
legend('Meas abs','meas abs pikebi')
grid on
...
for count_p = 1:1:number_of_elements+1
    ...
    for m = 1:1:length(eval(['p_',int2str(count_p_m)]))
        ...
        %% opt
        if count_p ~= number_of_elements+1
            for count_m = 1:1:length(count_index)
                
                x(1,count_m) = (abs((data_meas(count_index(count_m), 2)+1i*data_meas(count_index(count_m), 3)) - Z(count_index(count_m)))).^2;
                y(1,count_m) = (abs(data_meas(count_index(count_m), 2)+1i*data_meas(count_index(count_m), 3))).^2;
